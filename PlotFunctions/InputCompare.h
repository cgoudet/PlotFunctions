...
namespace ChrisLib {
  ...
  /** \brief Stores the content of boost configuration files.
     ...
     - 1 : Read a variable of given branches and plot the distribution in an histogram.
     \n
     Mandatory variables : Binning, varName
     \n
     Optionnal variables : varWeight, doTabular, saveRoot, doLabels, objName

     - 2 : Read values of a given branch and compute a function of them. Results is displayed in an histogram, 1 bin per branch.
     \n
     ...
     - 5 : Fill a TGraph errors with varName as Xaxis, varYName as Yaxis. Errors are 0 if unspecified.
     \n
     Mandatory variables : varName, varYName
     \n
     Optionnal variables : varErrX, varErrY, doTabular, saveRoot, doLabels

     - 6 : Split input TTrees into two subTrees which either pass or fail a selection.
     \n
     Mandatory variables : selectionCut
     \n

     - 7 : Plot TMatrix values on an histogram with coordinates as labels.
     \n
     Mandatory variables : objName
     \n
